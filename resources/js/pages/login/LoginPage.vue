...
function login(e) {
    loading.value = true
    error.value = ''
    axios.post('api/login', new FormData(e.target)).then((response) => {
        ...
        const redirect = params.get('redirectFrom');
    ...
    <section class="h-screen w-screen bg-cover flex justify-center items-center overflow-auto">
        <div class="rounded-xl  bg-opacity-20 backdrop-blur-sm p-6 m-auto shadow-lg shadow-[#00000080]">
            ...
            <v-form @submit.prevent="login" class="mt-4 min-w-64" theme="dark">
                <div class="mb-4">
                    <v-text-field theme="dark" name="email" color="blue-grey-lighten-2" :disabled="loading"
                        label="Correo electrónico" prepend-inner-icon="mdi-email" variant="solo-filled"
                        :hide-details="true"></v-text-field>
                    <v-text-field theme="dark" class="mt-4" :hide-details="error == ''" :error="error != ''"
                        :error-messages="error" name="password" color="blue-grey-lighten-2" :disabled="loading"
                        label="Contraseña" prepend-inner-icon="mdi-key" type="password"
                        variant="solo-filled"></v-text-field>
                </div>

                <v-btn type="submit" :loading="loading" class="w-full" color="orange-darken-4" variant="tonal">Iniciar
                    sesión</v-btn>
                <p class="mt-6 text-center text-gray-200 text-sm">¿No tienes una cuenta?</p>
