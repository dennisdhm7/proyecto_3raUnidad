<script>
    ...
    export default {
        ...
        methods: {
            load() {
                axios.get(`api/client-notifications?page=${this.page}&per_page=100`).then((response) => {
        ...
        mounted() {
            this.emitter.on("client-notifications-update", () => {
                this.load()
            })
            this.load()
        },
        ...
        <v-card min-width="300">
            <v-list class="px-2">
                <template v-for="(notification, index) in notifications">
                    <v-list-item class="py-3" v-if="notification.user != null" :prepend-avatar="notification.user.image == null ? 'src/user_placeholder.jpg' : notification.user.image" :subtitle="notification.content">
                        <template v-slot:title>
                            <small class="float-right text-gray-500">{{ Utils.datetime(notification.created_at, Utils.FULL_DATE_TIME) }}</small>
                            <v-list-item-title>
