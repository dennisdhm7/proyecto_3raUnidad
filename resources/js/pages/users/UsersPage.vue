...
const columns = [
    ...
    { index: 1, title: 'Nombre', key: 'name', minWidth: 250 },
    { index: 2, title: 'Contacto', key: 'email', minWidth: 250 },
    { index: 3, title: 'Estado', key: 'active', value: item => item.active ? 'Activa' : 'Inactiva' },
    { index: 4, title: 'Útimo acceso', key: 'last_access', minWidth: 140 },
...
function remove() {
    ...
    axios.post(`api/users/${destroy.value.id}`, data).then(() => {
...
function createOrUpdate(data) {
    ...
    //Realizar la petición
    axios.post(item.value.id == undefined ? 'api/users' : `api/users/${item.value.id}`, data).then((response) => {
...
function updateStatus(item) {
    ...
    //Realizar la petición
    axios.post(`api/users/status/${item.id}`, data).then((response) => {
        //Actualizar item en el frontend
        for (let i in items.value) {
            if (items.value[i].id == item.id) {
                ...
                message.value = {
                    open: true,
                    text: 'Se ha actualizado correctamente el estado del usuario'
                }
                break
            }
        }
...
onMounted(() => {
    headers.value = Utils.loadColumns(columns, 'users')
    //Mostrar identificación visual de petición al backend
    loading.value = true
...
<template>
    <section class="bg-cover">
        <!-- Layout principal -->
        <LayoutComponent title="Usuarios">
                ...
                <div class="flex gap-2 justify-between items-center mb-2">
                    ...
                    <v-dialog width="auto" scrollable v-model="dialog">
                        <template v-slot:activator="{ props: activatorProps }">
                            <v-btn :disabled="!Utils.isAuthorized('Agregar usuarios')" v-bind="activatorProps"
                        ...
                        <template v-slot:default="{ isActive }">
                            <v-card :loading="loading"
                                :title="item.id == undefined ? 'Agregar usuario' : 'Actualizar usuario'"
                                :subtitle="`Completa el formulario para ${item.id == undefined ? 'agregar un nuevo usuario' : 'actualizar los datos del usuario'}`">
                                <v-divider></v-divider>
                                <v-card-text class="px-4 md:min-w-[40rem] max-w-[400px]" style="height: 300px;">
                                    <UserForm @submit="createOrUpdate" :item="item" :loading="loading" />
                                </v-card-text>
                                <v-divider></v-divider>
                    ...
                    <div class="w-48 md:w-64">
                        <RowFilter v-model="search" id="users" :headers="headers" :columns="columns" />
                    </div>
                </div>

                <!-- Tabla con los datos  -->
                <v-data-table class=" rounded-md" v-model:search="search" :headers="headers"
                    :items="items" :loading="loading" hover items-per-page-text="Usuarios por página"
                    prev-page-label="Página anterior" next-page-label="Página siguiente" last-page-label="Última página"
                    first-page-label="Primera página" no-data-text="No se encontraron usuarios"
                    ...
                    <template v-slot:item="{ item }">
                        <tr>
                            <td v-if="Utils.isColumnVisibility(headers, columns[0].key)">
                                {{ Utils.getCode(item.id, 'U', 3) }}
                            </td>
                            <td v-if="Utils.isColumnVisibility(headers, columns[1].key)">
                                <v-list-item class="px-0" :title="item.name"
                                    :subtitle="`${item.id_admin == 1 ? 'Administrador principal' : 'Usuario estándar'}`"
                                    :prepend-avatar="item.image || '/src/user_placeholder.jpg'">

                                </v-list-item>
                            </td>

                            <td v-if="Utils.isColumnVisibility(headers, columns[2].key)">
                                <v-list-item class="px-0" :title="item.email" :subtitle="item.phone">
                                </v-list-item>
                            </td>
                            ...
                            <td v-if="Utils.isColumnVisibility(headers, columns[4].key)">
                                <template v-if="item.last_access == null">
                                    No disponible
                                </template>
                                <template v-else>
                                    {{ Utils.datetime(item.last_access, Utils.FULL_DATE_TIME) }}
                                </template>
                            </td>
                            ...
                            <td v-if="Utils.isColumnVisibility(headers, columns[6].key)">
                                <v-btn :disabled="!Utils.isAuthorized('Editar usuarios')"
                                    v-tooltip="'Cambiar estado del usuario'" icon="mdi-toggle-switch"
                                    @click="updateStatus(item)" size="x-small" class="me-1" rounded="lg"
                                    color="grey-lighten-1" variant="tonal">
                                </v-btn>
                                <v-btn :disabled="!Utils.isAuthorized('Editar usuarios')"
                                    v-tooltip="'Editar los datos del usuario'" icon="mdi-pencil" @click="edit(item)"
                                    size="x-small" class="me-1" rounded="lg" color="grey-lighten-1" variant="tonal">
                                </v-btn>
                                <v-btn :disabled="!Utils.isAuthorized('Eliminar usuarios')" v-tooltip="'Eliminar usuario'"
                                    icon="mdi-trash-can" @click="destroy = { open: true, id: item.id }" size="x-small"
                    ...
                    <template v-slot:no-data>
                        <EmptyData text="No hemos encontrado ningún usuario" />
                    </template>
                </v-data-table>
            </div>
        ...
        <ConfirmDialog @confirm="remove" v-model="destroy.open" action="Eliminando usuario" title="Confirmar eliminación"
            :loading="loading" question="¿Está seguro que desea eliminar este usuario?. Esta operación es irreversible."
            icon="mdi-trash-can" />
    </section>
</template>
<style scoped>
