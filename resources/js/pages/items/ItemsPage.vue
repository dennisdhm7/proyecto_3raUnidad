...
const columns = [
    ...
    { index: 1, title: 'Nombre', key: 'name', minWidth: 350 },
    { index: 2, title: 'Descripción', key: 'description', minWidth: 500 },
    { index: 3, title: 'Precio', key: 'local_price', minWidth: 150 },
    { index: 4, title: 'Estado', key: 'active', value: item => item.active ? 'Activa' : 'Inactiva' },
    { index: 5, title: 'Creación', key: 'created_at', minWidth: 140 },
    { index: 6, title: 'Acciones', key: 'actions', minWidth: 120, minWidth: 150, sortable: false },
]
...
function remove() {
    ...
    axios.post(`api/items/${destroy.value.id}`, data).then(() => {
        ...
        for (let i in items.value) {
            if (items.value[i].id == destroy.value.id) {
                ...
                message.value = {
                    open: true,
                    text: 'Se ha eliminado correctamente el producto o servicio'
                }
...
function createOrUpdate(data) {
    ...
    //Realizar la petición
    axios.post(item.value.id == undefined ? 'api/items' : `api/items/${item.value.id}`, data).then((response) => {
        if (item.value.id == undefined) {
            ...
            message.value = {
                open: true,
                text: 'Se ha creado correctamente el producto o servicio'
            }
            ...
            for (let i in items.value) {
                if (items.value[i].id == item.value.id) {
                    ...
                    message.value = {
                        open: true,
                        text: 'Se han actualizado correctamente los datos del producto o servicio'
                    }
                    break
                }
            }
...
function updateStatus(item) {
    ...
    //Realizar la petición
    axios.post(`api/items/status/${item.id}`, data).then((response) => {
        //Actualizar item en el frontend
        for (let i in items.value) {
            if (items.value[i].id == item.id) {
                ...
                message.value = {
                    open: true,
                    text: 'Se ha actualizado correctamente el estado del producto o servicio'
                }
                break
            }
        }
...
onMounted(() => {
    headers.value = Utils.loadColumns(columns, 'items')
    //Mostrar identificación visual de petición al backend
    loading.value = true
...
<template>
    <section class="bg-cover">
        <!-- Layout principal -->
        <LayoutComponent title="Productos o servicios">
            <div class="w-full min-h-[calc(100vh-100px)]">
                <div class="flex gap-2 justify-between items-center mb-2">
                    ...
                    <v-dialog width="auto" scrollable v-model="dialog" :transition="false">
                        <template v-slot:activator="{ props: activatorProps }">
                            <v-btn :disabled="!Utils.isAuthorized('Agregar productos o servicios')" v-bind="activatorProps"
                        ...
                        <template v-slot:default="{ isActive }">
                            <v-card :loading="loading"
                                :title="item.id == undefined ? 'Agregar producto o servicio' : 'Actualizar producto o servicio'"
                                :subtitle="`Completa el formulario para ${item.id == undefined ? 'agregar un nuevo producto o servicio' : 'actualizar los datos del producto o servicio'}`">
                                <v-divider></v-divider>
                                <v-card-text class="px-4 md:min-w-[40rem]" style="height: 300px;">
                                    <ItemForm @submit="createOrUpdate" :item="item" :loading="loading" />
                                </v-card-text>
                                <v-divider></v-divider>
                                ...
                                <v-card-actions>
                                    ...
                                    <v-btn color="blue"
                                        ...
                                        form="item-form"></v-btn>
                                </v-card-actions>
                            </v-card>
                        </template>
                    </v-dialog>
                    <div class="w-48 md:w-64">
                        <RowFilter v-model="search" id="items" :headers="headers" :columns="columns" />
                    </div>
                </div>

                <!-- Tabla con los datos  -->
                <v-data-table class=" rounded-md" v-model:search="search" :headers="headers"
                    :items="items" :loading="loading" hover items-per-page-text="Productos o servicios por página"
                    prev-page-label="Página anterior" next-page-label="Página siguiente" last-page-label="Última página"
                    first-page-label="Primera página" no-data-text="No se encontraron productos o servicios"
                    ...
                    <template v-slot:item="{ item }">
                        <tr>
                            ...
                            <td v-if="Utils.isColumnVisibility(headers, columns[3].key)">
                                <v-list-item class="px-0" :title="`${Utils.currency(item.local_price)}`">
                            ...
                            <td v-if="Utils.isColumnVisibility(headers, columns[6].key)">
                                <v-btn :disabled="!Utils.isAuthorized('Editar poductos o servicios')"
                                    v-tooltip="'Cambiar estado del poducto o servicio'" icon="mdi-toggle-switch"
                                    @click="updateStatus(item)" size="x-small" class="me-1" rounded="lg"
                                    color="grey-lighten-1" variant="tonal">
                                </v-btn>
                                <v-btn :disabled="!Utils.isAuthorized('Editar poductos o servicios')"
                                    v-tooltip="'Editar los datos del poducto o servicio'" icon="mdi-pencil"
                                    @click="edit(item)" size="x-small" class="me-1" rounded="lg" color="grey-lighten-1"
                                    variant="tonal">
                                </v-btn>
                                <v-btn :disabled="!Utils.isAuthorized('Eliminar poductos o servicios')"
                                    v-tooltip="'Eliminar poducto o servicio'" icon="mdi-trash-can"
        ...
        <ConfirmDialog @confirm="remove" v-model="destroy.open" action="Eliminando poducto o servicio"
            title="Confirmar eliminación" :loading="loading"
            question="¿Está seguro que desea eliminar este poducto o servicio?. Esta operación es irreversible."
            icon="mdi-trash-can" />
    </section>
</template>
<style scoped>
